!*****************************************************************************
subroutine  NPChain_StateChain_A( state_chain )
!*****************************************************************************
!
...
!  begin subroutine
      ...
      if ( debug ) write(*,*) 'Entering NPChain_StateChain_A()'
...
! loop over replicas, advancing momenta by half a step

      do i = 1, nReplicas

         ! get position, momentum and force from state
      ...
      do i = 1, nReplicas
         ...
         do j = 1, nAtoms
            ...
            kinetic = kinetic + factor *   &
                 dot_product( momentum_chain(1:3,j,i), momentum_chain(1:3,j,i) )
      ...
      do i = 1, nReplicas
         ...
         do j=1, nAtoms
            ...
            position_chain(1:3,j,i) = position_chain(1:3,j,i) + &
                 factor * momentum_chain(1:3,j,i) / mass(j)
...
! finally, store the new coordinates and half-step momenta in state
      ...
      do i = 1, nReplicas
         ...
         call set( state_replica(i), position = position_chain(:,:,i), momentum = momentum_chain(:,:,i) )

         call setStateChain( state_chain, iReplica = i, state = state_replica(i) )

      end do

      if ( debug ) write(*,*) 'Exiting NPChain_StateChain_A()'
...
!  local variables
      ...
      logical :: energyConverged
      logical, dimension (:), allocatable :: converged

      integer :: m, n
      integer :: nIterations
      integer :: nEnergyIterations

      real (dp) :: deltaH
      real (dp) :: kbT
      real (dp) :: prod
      real (dp) :: thermostatEnergyNew, thermostatEnergyTemp
      real (dp) :: thermostatProduct

      real (dp), parameter :: energyTolerance = 1.0e-10_dp
...
end subroutine NPChain_StateChain_A
